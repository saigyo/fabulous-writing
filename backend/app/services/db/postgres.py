...
class PostgresDatabase:
    ...
    def __init__(self, dsn: str) -> None:
        ...
        try:
            ...
            # wait() already closed the pool on timeout (psycopg_pool's own
            # behavior) — no double-close needed here. The bare PoolTimeout
            # only says "30s elapsed"; the connection errors it logged along
            # the way carry the actual cause, so point the operator there
            # instead of repeating the generic timeout.
            raise RuntimeError(
                f"could not connect to the database from {DATABASE_URL_ENV} "
                "within 30s; see the logged connection errors for the cause"
            ) from exc
