...
In `README.md`'s Ollama troubleshooting bullet (~lines 206-213): add that this applies to commercial-provider setups too (the local tier probes host Ollama), and that on ALL platforms Ollama must listen beyond its `127.0.0.1` default for the container to reach it — `OLLAMA_HOST=0.0.0.0` (or an interface-specific bind) before starting Ollama. One more sentence in the backend-architecture note: on commercial configs the URL has no prompt, so correcting a stale hand-edited value means editing `config.yaml` directly (or one round-trip through the Ollama provider).
