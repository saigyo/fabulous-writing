...
| B8 | **Backend test-suite execution time.** Dev-technical (owner request 2026-07-27): >1000 tests now take ~4 minutes, and agent-driven workflows run the suite many times per task — the cost compounds. Owner's reference discussion: https://claude.ai/share/7764ffc7-2bf4-4b3a-98d6-acea7fba576e. Measure first (`pytest --durations=25`), then the usual levers for this stack, in expected-impact order: (1) `pytest-xdist -n auto` — tests are already `tmp_path`-isolated per test, so parallelization should be near-free; audit for hidden shared state (env vars, fixed ports — the ASGI-transport tests bind none) rather than assuming. (2) bcrypt cost in tests — every login/seeded-admin pays full production hashing; a conftest-scoped low-cost setting (or cheap hasher injection) typically saves minutes across hundreds of auth-touching tests, with one guard test keeping the production cost pinned. (3) Fixture scoping — `create_app` per test re-runs migrations/seeders/NLP registry setup; widen scope where isolation provably permits (mind the project's create_app history: bare `create_app()` must never touch the live DB, and widened fixtures must not reintroduce cross-test state). (4) Audit real `sleep`s/time-based waits (backpressure/throttle tests) for injectable clocks. | Not user-facing, so it never outranked a milestone; but the 4-minute wall multiplies into every fix round's gate. | Any gap between milestones; independently of M6. Re-measure after each lever — stop when the suite is comfortably under a minute rather than chasing the long tail. |
