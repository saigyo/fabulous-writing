...
| M6 | `multi-user-m6-admin-ui` — **DONE** (PR [#33](https://github.com/saigyo/fabulous-writing/pull/33), 2026-07-28; plan PR #32) | Admin view as a fifth `activeView`, gated on `is_admin`; user table with create/edit/deactivate/reset. Shipped with one recorded deviation from the row's original "frontend-only" framing: `GET /api/admin/tiers` (config tier names for the create/edit selects). | Purely additive view behind `is_admin`; non-admin sessions issue zero `/api/admin/*` requests by construction. |
...
| B3 | **A proper dialog pattern.** M2 puts the change-password form inline in the account popover. **Also in this pass (from PR #33, M6, 2026-07-28):** the admin view's own-row password reset works but ends the session abruptly — `set_password` bumps `token_epoch` (the M2 revocation rule), so the next request 401s to the login gate with no warning. Either disable self-reset on the own row (pointing at the Account menu) or warn before the committed reset. | A scrim already exists — `FolderDefaultsDialog` renders `.dialog-overlay` (`App.css:1719`) with backdrop-click dismissal — but focus trap, Escape handling and scroll lock do not. Three short fields did not justify building those, and a half-accessible dialog is worse than a popover. The self-reset note is deliberate M6 behavior (revocation semantics are correct; only the UX is rough), so it waits for the same polish pass rather than a review patch. | A UI polish phase: harden `.dialog-overlay` into a reusable accessible dialog, then move the password form onto it; handle the admin own-row reset UX in the same pass. |
...
| B8 | **Backend test-suite execution time — core levers DONE** (PR #31, 2026-07-28, spec `docs/superpowers/specs/2026-07-28-backend-test-speedup-design.md`): bcrypt cost 4 in the test process (guard tests pin test cost 4 and production cost 12), deterministic `document_clock` fixture replaced the eight 1.1 s sleeps, `pytest-xdist -n auto --dist load` on by default (`-n0` escape hatch; `loadfile` measured and rejected: 47 s with a hard 40 s single-file floor). Measured: 245.28 s serial baseline → 99.3 s serial → **28.2–29.0 s parallel**, 1,082 tests, zero warnings; the exact CI coverage command verified under xdist. **Remaining levers (deferred):** per-test schema/app cost (session-scoped schema + transaction rollback — mind the bare-`create_app()` history), CI-specific work (uv dependency caching, `COVERAGE_CORE=sysmon` on 3.12+; re-measure CI after PR #31 lands), test-helper `ResourceWarning` cleanups (8 sites, visible under `-W error` and `--cov`), inner-loop tools (`--lf`, `pytest-testmon`). Watch item: six wall-clock assertions (`test_check_api.py` `< 0.3 s` etc.) under 4-core CI contention — never widen a bound; de-clock or pin serial instead. | Core levers delivered as the 2026-07-28 emergency intervention between M5 and M6. | Remaining levers: only if CI stays > ~3 min after PR #31, or when `-W error` adoption is wanted. |
