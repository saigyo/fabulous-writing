...
describe('ruleDetailSummary', () => {
  it('lists existence tokens and raw patterns', () => {
    expect(
      ruleDetailSummary(
        rule({ detail: { tokens: ['very', 'fairly'], raw: ['!{2,}'], ignorecase: true } }),
      ),
  ...
  it('truncates long token lists', () => {
    ...
    expect(ruleDetailSummary(rule({ detail: { tokens, raw: [] } }))).toBe(
  ...
  it('shows substitution swaps as arrows', () => {
    expect(
      ruleDetailSummary(
        rule({ extends: 'substitution', detail: { swap: { utilize: 'use' } } }),
      ),
    ).toBe('utilize → use')
  })
  ...
  it('describes repetition and pattern rules', () => {
    expect(ruleDetailSummary(rule({ extends: 'repetition' }))).toBe(
      'Adjacent repeated words',
    )
    expect(
      ruleDetailSummary(
        rule({ extends: 'token_pattern', detail: { pattern: [{}, {}], suggestions: [] } }),
      ),
    ).toBe('spaCy token pattern (2 tokens)')
    expect(
      ruleDetailSummary(
        rule({ extends: 'dependency', detail: { pattern: [{}, {}], suggestions: [] } }),
      ),
